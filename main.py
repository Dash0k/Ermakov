...
count_marks_pi101 = len(df[df['Группа'] == 'ПИ101'])
count_student_pi101 = df.loc[df['Группа'] == 'ПИ101', 'Личный номер студента'].nunique()

mass_id_stud_pi101 = df.loc[df['Группа'] == 'ПИ101', 'Личный номер студента'].unique()
id_stud_pi101 = ", ".join(map(str, mass_id_stud_pi101))

mass_forms_control = df.loc[df['Группа'] == 'ПИ101', 'Уровень контроля'].unique()
forms_control = ", ".join(mass_forms_control)

all_years1 = df.loc[df['Группа'] == 'ПИ101','Год'].unique()
